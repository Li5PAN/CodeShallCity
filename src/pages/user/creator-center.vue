        ...
        <div class="creator-card">
          ...
          <div class="data-grid">
            <div class="data-item" v-for="d in dataStats" :key="d.label">
              <component
                :is="d.icon"
                :style="{ color: d.color, fontSize: '20px' }"
              />
              <div class="data-num" :style="{ color: d.color }">
                {{ d.value }}
              </div>
            ...
            <div class="recent-item" v-for="item in recentItems" :key="item.id">
              <div class="recent-info">
                ...
                <div class="recent-meta">
                  <a-tag size="small" :color="item.statusColor">{{
                    item.statusText
                  }}</a-tag>
                  <span>{{ item.time }}</span>
                </div>
              ...
              <div class="recent-stats">
                <span v-if="item.reads !== undefined"
                  ><EyeOutlined /> {{ item.reads }}</span
                >
                <span v-if="item.orders !== undefined"
                  ><ShoppingCartOutlined /> {{ item.orders }}</span
                >
                <span v-if="item.applies !== undefined"
                  ><TeamOutlined /> {{ item.applies }}</span
                >
              </div>
            </div>
      ...
      <div class="creator-side">
        <div class="creator-card provider-card">
          ...
          <a-button
            ...
            type="primary"
            block
            style="background: #52c41a; border-color: #52c41a; margin-top: 12px"
...
const notices = ref([
  {
    ...
    tag: "平台公告",
    content:
      "平台服务协议已更新，请创作者查阅最新版本，新版本将于 2026-04-01 正式生效。",
    time: "2026-03-01",
  },
  {
    id: 2,
  ...
  },
  {
    ...
    tag: "系统通知",
    content:
      "服务市场新增「AI应用开发」分类，欢迎相关服务商入驻，享受首月零佣金优惠。",
    time: "2026-03-03",
  },
]);
...
.notice-bar :deep(.ant-carousel) {
  width: 100%;
}
.notice-bar :deep(.slick-slide) {
  height: 44px;
