    ...
    <div class="service-list">
      ...
      <div class="service-item" v-for="item in filteredList" :key="item.id">
        ...
        <div class="service-actions">
          <a-button size="small" @click="openPublishDrawer(item)"
            >编辑</a-button
          >
          <a-button
            size="small"
...
const services = ref([
  ...
  {
    id: 3,
    title: "Python 数据分析实战项目",
    desc: "基于真实业务数据，手把手带你完成数据清洗、分析、可视化全流程",
    price: 199,
    cover: "https://placehold.co/120x80/3776AB/FFFFFF?text=Python",
    tags: ["一对一辅导"],
    category: "Python",
    status: "off",
    orders: 42,
  ...
  {
    id: 5,
    title: "MySQL 性能优化深度实战",
    desc: "从底层原理到实战优化，涵盖索引、事务、锁机制、分库分表",
    price: 349,
    cover: "https://placehold.co/120x80/FF6600/FFFFFF?text=MySQL",
    tags: ["平台保障", "售后答疑"],
    category: "数据库",
    status: "on",
    orders: 30,
...
const filteredList = computed(() => {
  ...
  if (searchKeyword.value.trim())
    list = list.filter((s) => s.title.includes(searchKeyword.value.trim()));
  return list;
});
...
onMounted(() => {
  if (route.query.action === "create") openPublishDrawer();
});
...
const onCoverChange = (e) => {
  ...
  reader.onload = (ev) => {
    form.coverUrl = ev.target.result;
  };
