          ...
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'action'">
              <div class="action-buttons">
                <a-button type="link" size="small" @click="viewServiceDetail(record)">查看</a-button>
              </div>
            </template>
          </template>
        </a-table>
      ...
      <a-tab-pane key="demand" tab="需求审核">
        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'action'">
              <div class="action-buttons">
                <a-button type="link" size="small" @click="viewDemandDetail(record)">查看</a-button>
              </div>
            </template>
          </template>
        </a-table>
      ...
      <div v-if="serviceDetail.data" class="service-detail-modal">
        <div class="product-section">
          <div class="product-gallery">
            <div class="main-image">
              <img :src="serviceDetail.data.images[serviceDetail.currentImage]" alt="服务图" />
            </div>
            <div class="thumb-list" v-if="serviceDetail.data.images?.length > 1">
              <div
          ...
          <div class="product-info">
            ...
            <div class="price-box">
              <div class="price-row">
                ...
                <span class="price-value">¥ {{ serviceDetail.data.price }}</span>
              </div>
              <div class="price-row">
                <span class="price-label">销量</span>
                <span class="price-info">{{ serviceDetail.data.sales }}+ 笔</span>
              </div>
              <div class="price-row">
                <span class="price-label">好评率</span>
                <span class="price-info" style="color: #52c41a">{{ serviceDetail.data.rating }}%</span>
              </div>
            </div>
            <div class="guarantee-tags">
              <span class="price-label">保障</span>
              <div class="tag-list">
                <a-tag v-for="tag in serviceDetail.data.tags" :key="tag" color="blue">
                  <CheckCircleOutlined /> {{ tag }}
                </a-tag>
              </div>
            </div>
            <div class="seller-info">
              <a-avatar :size="40" :style="{ backgroundColor: serviceDetail.data.providerColor }">
        ...
        <div class="detail-body">
          <div class="detail-left">
            <a-tabs v-model:activeKey="serviceDetail.activeTab" class="detail-tabs">
              <a-tab-pane key="detail" tab="服务详情">
                <div class="detail-content">
                  ...
                  <h3>服务内容</h3>
                  <ul>
                    <li v-for="(item, idx) in serviceDetail.data.detailItems" :key="idx">{{ item }}</li>
                  </ul>
                  <h3>交付标准</h3>
          ...
          <div class="detail-card audit-card">
            <div class="audit-action-bar">
              ...
              <div class="audit-action-buttons">
                <a-button type="primary" style="background-color: #52c41a; border-color: #52c41a" @click="handleServiceAudit('approve')">
                  通过
                </a-button>
    ...
    >
      <div v-if="demandDetail.data" class="demand-detail-modal">
        ...
        <div class="detail-card">
          ...
          <div class="meta-line">
            <span class="meta-author">发布者：{{ demandDetail.data.publisher }}</span>
            <span class="meta-time">发布时间：{{ demandDetail.data.submitTime }}</span>
            <span class="meta-time">截止时间：{{ demandDetail.data.deadline }}</span>
            <a-tag :color="demandDetail.data.status === 'pending' ? 'orange' : (demandDetail.data.status === 'approved' ? 'green' : 'red')" class="status-tag">
          ...
          <div class="info-grid">
            ...
            <div class="info-row">
              ...
              <span class="info-value price-text">
                ¥ {{ demandDetail.data.budgetMin }} ~ {{ demandDetail.data.budgetMax }}
              </span>
            </div>
            <div class="info-row">
              <span class="info-label">需求描述</span>
              <span class="info-value">{{ demandDetail.data.description }}</span>
            </div>
            <div class="info-row">
            ...
            <div class="info-row">
              ...
              <span class="info-value">
                <a-tag :color="getUrgencyColor(demandDetail.data.urgency)">{{ demandDetail.data.urgency }}</a-tag>
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="detail-card detail-intro-card">
          ...
          <div class="detail-text">{{ demandDetail.data.detailDescription }}</div>

          <h3 class="card-sub-title" style="margin-top: 20px">功能要求清单</h3>
          <ul class="requirement-list">
            <li v-for="(item, idx) in demandDetail.data.requirementItems" :key="idx">{{ item }}</li>
          </ul>
        </div>
        ...
        <template v-if="demandDetail.data.status === 'pending'">
          <div class="detail-card audit-card">
            <div class="audit-action-bar">
              ...
              <div class="audit-action-buttons">
                <a-button type="primary" style="background-color: #52c41a; border-color: #52c41a" @click="handleDemandAudit('approve')">
                  通过
                </a-button>
...
import { CheckCircleOutlined, TrophyOutlined, EyeOutlined, FileTextOutlined } from '@ant-design/icons-vue'
...
const serviceList = ref([
  {
    ...
    detailIntro: '本课程由资深前端架构师倾力打造，从零开始带你搭建企业级Vue3项目。涵盖Vue3核心特性、Composition API、TypeScript类型系统、Pinia状态管理、Vue Router路由管理等核心技术。',
    detailItems: ['Vue3核心特性和Composition API详解', 'TypeScript在Vue3中的最佳实践', 'Pinia状态管理从入门到精通', 'Vue Router4路由管理与权限控制', 'Element Plus组件库深入源码分析', 'Vite构建工具原理与性能优化', '企业级项目架构设计与分层', '前后端分离项目实战'],
    deliveryStandard: '购买后24小时内开通课程权限，配套源码永久有效，提供1年技术答疑服务。'
  },
  {
    id: 2,
    name: 'Python爬虫数据采集服务',
    ...
    rating: 99,
    tags: ['平台保障', '数据安全', '专业团队'],
    detailIntro: '专业的数据采集团队，提供全网数据采集服务。覆盖电商、社交、新闻、论坛等各类网站，支持动态渲染页面、验证码识别、反爬虫绕过等高级功能。',
    detailItems: ['静态网页数据采集', '动态渲染页面（JS渲染）数据采集', '反反爬虫策略定制', '验证码识别与处理', '数据清洗与格式化', '大规模分布式采集架构'],
    deliveryStandard: '约定时间内交付数据，支持多种格式（Excel、CSV、JSON、数据库），提供数据质量保证。'
  },
  {
    id: 3,
    name: 'Java微服务架构设计',
    ...
    rating: 97,
    tags: ['平台保障', '架构咨询', '源码交付'],
    detailIntro: '10年+架构师经验，专注微服务架构设计与实现。基于Spring Cloud Alibaba生态，提供从需求分析、架构设计到落地实施的全程服务。',
    detailItems: ['微服务架构设计咨询', 'Spring Cloud Alibaba全家桶实现', 'Nacos注册中心与配置中心', 'Sentinel流量控制与熔断降级', 'Seata分布式事务解决方案', 'Gateway统一网关设计', 'Skywalking全链路追踪', 'Docker/K8s容器化部署'],
    deliveryStandard: '提供完整源码、架构文档和部署手册，支持一对一技术讲解，售后提供3个月免费维护。'
  },
  {
    id: 4,
    name: 'AI大模型应用开发',
    ...
    detailIntro: '专注于AI大模型应用开发，涵盖GPT、Claude、国产大模型的接入与应用。提供从Prompt工程、RAG知识库搭建到Agent智能体开发的全栈服务。',
    detailItems: ['GPT/Claude/文心/通义等大模型接入', 'Prompt工程与调优实战', 'RAG知识库构建与优化', 'LangChain框架应用开发', 'AI Agent智能体设计与实现', '向量数据库选型与部署', '企业级AI应用架构设计', '私有化大模型部署方案'],
    deliveryStandard: '交付完整项目源码和部署文档，提供AI应用最佳实践指导，售后服务期限为6个月。'
  },
  {
    id: 5,
    ...
    status: 'rejected',
    images: [
      'https://placehold.co/480x360/61DAFB/000000?text=RN封面'
    ],
    sales: 450,
    rating: 95,
    tags: ['平台保障', '跨平台', '源码交付'],
    detailIntro: '使用React Native开发跨平台移动应用，一套代码同时支持iOS和Android。包含用户系统、支付功能、消息推送、数据统计等完整功能模块。',
    detailItems: ['React Native环境搭建与项目初始化', 'React Navigation导航系统', 'Redux/MobX状态管理', 'iOS/Android原生模块开发', '第三方SDK集成（支付、推送、统计）', '应用商店上架指导'],
    deliveryStandard: '交付完整源码和上架指南，提供7x24小时答疑服务，协助解决上架问题。'
  }
])
...
const demandList = ref([
  {
    ...
    urgency: '紧急',
    detailDescription: '需要基于MiniMax大模型开发一套智能客服系统，支持多轮对话、意图识别、知识库问答等功能，需提供完整源码及部署文档。\n\n功能要求：\n1. 多轮对话管理，支持上下文理解\n2. 意图识别与自动分类\n3. 知识库问答，支持FAQ批量导入\n4. 对话记录查询与统计分析\n5. 支持接入微信、钉钉等主流平台',
    requirementItems: ['多轮对话系统设计与实现', '意图识别模型训练与部署', '知识库管理与问答匹配', '后台管理系统开发', 'API接口文档与部署文档', '提供6个月免费维护服务']
  },
  {
    id: 2,
    ...
    urgency: '一般',
    detailDescription: '开发一款跨平台移动应用，使用React Native技术栈，一套代码同时支持iOS和Android。\n\n功能模块：\n1. 用户注册登录（手机号+验证码）\n2. 商品展示与搜索\n3. 购物车与订单管理\n4. 微信/支付宝支付接入\n5. 用户中心与消息通知',
    requirementItems: ['React Native项目初始化与环境配置', 'React Navigation导航系统', 'Redux状态管理', 'iOS/Android双平台打包', '微信支付与支付宝支付集成', '应用商店上架指导']
  },
  {
    id: 3,
    title: 'Vue3企业级后台管理系统',
    ...
    urgency: '紧急',
    detailDescription: '开发一套完整的企业级Vue3后台管理系统，采用前后端分离架构。\n\n技术要求：\n1. 基于Vue3 + TypeScript + Vite\n2. 使用Pinia进行状态管理\n3. Element Plus组件库\n4. JWT Token身份认证\n5. RBAC权限控制\n6. ECharts数据可视化',
    requirementItems: ['项目架构设计与技术选型', '用户管理模块（含RBAC权限）', '菜单管理与动态路由', '数据可视化大屏开发', '系统日志与操作记录', '接口文档与部署手册']
  },
  {
    id: 4,
    title: '电商数据分析与可视化',
    ...
    urgency: '常规',
    detailDescription: '对电商平台的销售数据进行深度分析，开发数据可视化系统，支持实时监控和趋势预测。\n\n分析内容：\n1. 销售数据分析（GMV、订单量、客单价）\n2. 用户行为分析（访问路径、转化率）\n3. 商品分析（爆款、滞销、关联销售）\n4. 库存与供应链分析\n5. 销售预测模型',
    requirementItems: ['数据仓库设计与ETL开发', '销售数据指标体系搭建', '多维度数据可视化大屏', '用户行为分析模型', '销售预测机器学习模型', '数据导出与报告生成功能']
  }
])
...
const handleServiceAudit = (type) => {
  ...
  const record = serviceList.value.find(item => item.id === serviceDetail.value.data?.id)
  if (record) {
...
const handleDemandAudit = (type) => {
  ...
  const record = demandList.value.find(item => item.id === demandDetail.value.data?.id)
  if (record) {
    record.status = type === 'approve' ? 'approved' : 'rejected'
    message.success(type === 'approve' ? '审核通过' : '已驳回')
  }
...
.detail-cover { width: 120px; height: 90px; border-radius: 8px; object-fit: cover; flex-shrink: 0; }
.demand-icon-large { width: 120px; height: 90px; display: flex; align-items: center; justify-content: center; background: #fafafa; border-radius: 8px; flex-shrink: 0; }
.detail-header-info { flex: 1; min-width: 0; }
.detail-title { font-size: 16px; font-weight: 700; color: #1f2937; margin-bottom: 8px; }
...
/* 服务详情弹窗样式 */
.service-detail-modal { width: 100%; }
.product-section { display: flex; gap: 24px; margin-bottom: 16px; }
.product-gallery { width: 420px; flex-shrink: 0; }
.main-image { width: 100%; height: 280px; border-radius: 8px; overflow: hidden; background: #f5f5f5; margin-bottom: 12px; }
.main-image img { width: 100%; height: 100%; object-fit: cover; }
.thumb-list { display: flex; gap: 8px; }
.thumb-item { width: 60px; height: 45px; border-radius: 4px; overflow: hidden; cursor: pointer; border: 2px solid transparent; opacity: 0.6; transition: all 0.2s; }
.thumb-item.active { border-color: #1890ff; opacity: 1; }
.thumb-item:hover { opacity: 1; }
.thumb-item img { width: 100%; height: 100%; object-fit: cover; }
.product-info { flex: 1; min-width: 0; }
.product-title { font-size: 18px; font-weight: 700; color: #333; margin: 0 0 8px 0; line-height: 1.4; }
.product-subtitle { font-size: 14px; color: #666; margin: 0 0 12px 0; }
.price-box { background: #fff7f0; border-radius: 8px; padding: 12px; margin-bottom: 12px; }
...
.price-value { font-size: 24px; font-weight: 700; color: #ff4d4f; }
.price-info { font-size: 14px; color: #333; }
.guarantee-tags { display: flex; align-items: flex-start; gap: 12px; margin-bottom: 12px; padding: 8px 0; border-bottom: 1px solid #f0f0f0; }
.tag-list { display: flex; gap: 8px; flex-wrap: wrap; }
.seller-info { display: flex; align-items: center; gap: 12px; padding: 12px 0; }
...
.detail-left { width: 100%; background: #fafafa; border-radius: 8px; padding: 16px; }
.detail-content h3 { font-size: 15px; font-weight: 600; color: #333; margin: 16px 0 10px 0; padding-left: 10px; border-left: 3px solid #1890ff; }
.detail-content h3:first-child { margin-top: 0; }
.detail-content p { font-size: 14px; color: #666; line-height: 1.8; margin: 0 0 12px 0; }
.detail-content ul { padding-left: 20px; margin: 0 0 14px 0; }
.detail-content li { font-size: 14px; color: #666; line-height: 1.8; }
.audit-card { background: #fafafa; padding: 20px; }

/* 需求详情弹窗样式 */
.demand-detail-modal { width: 100%; }
.detail-card { background: #fff; border-radius: 8px; padding: 24px; margin-bottom: 16px; }
.card-title { font-size: 18px; font-weight: 700; color: #333; margin: 0 0 16px 0; }
.card-sub-title { font-size: 15px; font-weight: 600; color: #333; margin: 0 0 12px 0; padding-left: 10px; border-left: 3px solid #1890ff; }
.meta-line { display: flex; align-items: center; gap: 16px; margin-bottom: 20px; font-size: 13px; color: #999; flex-wrap: wrap; }
.meta-author { color: #666; font-weight: 500; }
.meta-time { color: #999; }
.status-tag { margin-left: auto; }
.info-grid { display: flex; flex-direction: column; border: 1px solid #f0f0f0; border-radius: 6px; overflow: hidden; margin-bottom: 16px; }
.info-row { display: flex; align-items: center; padding: 12px 16px; border-bottom: 1px solid #f0f0f0; }
...
.stats-row { display: flex; gap: 32px; padding: 14px 16px; background: #fafafa; border-radius: 6px; margin-bottom: 16px; }
.stat-item { display: flex; align-items: center; gap: 8px; }
.stat-icon { font-size: 16px; color: #1890ff; }
.stat-label { font-size: 13px; color: #999; }
.stat-value { font-size: 16px; font-weight: 600; color: #333; }
.detail-intro-card { background: #fafafa; }
.detail-text { font-size: 14px; color: #666; line-height: 1.8; white-space: pre-line; margin-bottom: 8px; }
.requirement-list { padding-left: 20px; margin: 0; }
.requirement-list li { font-size: 14px; color: #666; line-height: 2; }
</style>
